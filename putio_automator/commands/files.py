...
@click.pass_context
@click.option(
    "--parent-id", help="ID of folder to list, defaults to root folder", type=int
)
...
@click.option(
    "--chunk-size", default=256, help="Chunk size for downloading files in kilobytes"
)
@click.option(
    "--parent-id",
    help="ID of folder to download from, defaults to root folder",
    type=int,
)
@click.option("--folder", default="", help="Local subfolder to store downloads")
@click.option(
    "--force",
    help="Force download if it has been seen before, ignores database",
    is_flag=True,
)
    ...
    if len(files):
        ...
        def func(connection):
            ...
            for current_file in files:
                conn.execute(
                    "select datetime(created_at, 'localtime') from downloads where name = ? and size = ?",
                ...
                if row is None or force:
                    ...
                    conn.execute(
                        "insert into downloads (id, name, size) values (?, ?, ?)",
                    ...
                    logger.warning(
                        "file already downloaded at %s : %s"
