    ...
    if len(files):
        ...
        def func(connection):
            ...
            for name in files:
                ...
                conn.execute(
                    "select datetime(created_at, 'localtime') from torrents where name = ? and size = ?",
                    (name, size),
                )
                row = conn.fetchone()
                ...
                if row is None:
                    try:
                        ...
                        if (
                            info[0] == putiopy.ClientError
                            and info[1].type == "UnknownError"
                        ):
                            # Assume it's already added
                            os.unlink(path)
                            logger.warning(
                                "deleted torrent, already added : %s" % (name,)
                            )
                        else:
                            raise
...
@click.option(
    "--sleep", default=5, help="Amount of seconds to sleep between filesystem polls"
)
@click.option(
    "--wait-for-closed",
    default=5,
    help="Amount of seconds to wait after created event to process a torrent",
)
